<template>
    <div class="container">
        <v-app hide-details>
            <div class="p-2 title">
                <h2 style="display: inline-block">Maternity Admission</h2>
                <v-btn class="float-right d-print-none" @click="print"><v-icon>mdi-printer</v-icon></v-btn>
            </div>
            <div class="table-responsive p-4">
                <table id="patient_gl_info">
                    <tr>
                        <th colspan="4" style="text-align:center;">General information about the patient</th>
                    </tr>
                    <tr>
                        <td style="width:10%">Id<span class="text-danger">*</span>  <input v-model="formData.patient_id" type="number" @change="change_patient" :class="{'error':$v.formData.patient_id.$error}"/></td>
                        <td>FirstName <input v-model="formData.firstName" type="text"/></td>
                        <td>LastName <input v-model="formData.lastName" type="text"/></td>
                        <td style="width:15%">BirthDate<input v-model="formData.birthDate" type="text"style="width:125px"/></td>
                    </tr>
                </table>
                ...
                <table id="patient_admission_details">
                    <tr>
                        <th style="text-align:center;" colspan="3">Admission details</th>
                    </tr>
                    <tr>
                        <td colspan="2">Date:<span class="text-danger">*</span> <input v-model="formData.date" type="date" style="margin-right:10px" :class="{'error':$v.formData.date.$error}"/>
                            | &nbspTime:<span class="text-danger">*</span> <input v-model="formData.time" type="time" :class="{'error':$v.formData.time.$error}" /></td>
                        <td>
                            <span class="mr-2">Service:</span><span class="text-danger">*</span> <select :class="{'error':$v.formData.hospital_service_division_id.$error}" v-model="formData.hospital_service_division_id" @change="fetch_free_bed"><option v-for="item in accessory.hospital_service_division" :value="item.id" >{{item.name}}</option></select>
...
const {
    ...
    export default {
        ...
        validations(){
            return{
                formData:{
                    ...
                    date:{required},
                    time:{required},
                    hospital_service_division_id:{required},
                    bed_id:{required},
                    ga:{required},
        ...
        methods:{
            ...
            async init(){
                let hosp_div=await axios.get('/api/hospital_service_division')
                this.accessory.hospital_service_division=hosp_div.data
            ...
            async fetch_free_bed(){
                await axios.get('/api/free_bed/'+this.formData.hospital_service_division_id).then(response=>{
