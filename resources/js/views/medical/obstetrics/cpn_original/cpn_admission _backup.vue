    ...
    <v-app class="p-4">
      <div class="d-flex flex-row justify-content-between">
        <div>
          ...
          <div class="row align-items-center" v-if="!is_overview">
            ...
            <div class="col-2" v-if="accessory.edit">
              <input
                type="number"
                class="form-control"
            ...
            <div
              class="col"
              v-if="!is_overview && accessory.edit && senior_auth_required"
            >
              <button
                type="button"
                class="btn btn-info"
                @click="accessory.auth_dialog = true"
              >
          ...
          <div class="mb-4 ml-4">
            <div class="row">
              <span class="error">{{
                accessory.ultrasound_link_error_message
              }}</span>
            </div>
            <div
              class="row d-flex"
            ...
            >
              ...
              <input
                min="0"
                class="ml-1 col-3 form-control"
                disabled
                type="number"
                v-model="formData.ultrasound_admission_id"
              />
              <div v-if="accessory.linked_ultrasound_exists">
                <router-link
                  ...
                  :to="{
                    ...
                    params: { ref: formData.ultrasound_admission_id },
                  }"
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                {{
                  patient_details.dob
                    ? new Date(patient_details.dob).toDateString()
                    : ""
                }}
                <span
                  class="error"
                  v-if="
                    $v.patient_details.$error &&
                    !$v.patient_details.dob.required
                  "
                  >D.O.B has no value</span
                >
              </td>
              <td>
                <span
                  ...
                  <span
                    v-if="
                      $v.patient_details.$error &&
                      !$v.patient_details.dob.validateAge
                    "
        ...
        <table class="table table-bordered" id="pregnancy_details_table">
          <tbody>
            ...
            <tr>
              ...
              <td>
                <span class="text-center">{{ formData.gestational_age }}</span>
              </td>
      ...
      <div class="row mt-3">
        <div class="col-12">
          <button
            ...
            v-if="accessory.edit && !is_overview"
...
export default {
  ...
  watch: {
    ...
    "$route.params": {
      handler: async function (val) {
        if (
          !this.accessory.initialized ||
          this.$route.params.id == this.accessory.reference ||
          !this.$route.params.id
        )
          return;

        this.reset_form();
        this.accessory.reference = val.id;
  ...
  methods: {
    async change_patient() {
      this.accessory.loading.patient = true;
      let response = await axios.get(
        `/api/v1/patient_system/patient/patient/${this.formData.patient_id}/edit`
      );
      ...
      if (response.data.patient) {
        ...
        let lastName =
          response.data.patient.lastName == null
            ? ""
            : response.data.patient.lastName;
        this.patient_details.firstName =
          response.data.patient.firstName.toUpperCase();
      ...
      this.accessory.loading.patient = false;
    },
    add_row() {
      ...
      if (!this.$v.$invalid) {
        if (this.accessory.edit === false) {
          let response = await axios.post(
            "/api/v1/patient_system/out_patient/obstetrical/cpn_admission",
            this.formData
          );
    ...
    async init() {
      ...
      const promises = [
        axios
          .get("/api/v1/extra/blood_group")
          .then((response) => (this.accessory.blood_group = response.data)),
        axios
          .get("/api/v1/extra/pregnancy_problem")
          .then((response) => (this.accessory.birth_problems = response.data)),
      ];
      ...
      if (this.$route.params.id !== undefined) {
        ...
        history.replaceState(
          null,
          null,
          this.$router.resolve({ name: "cpn_admission" }).href
        );
      }
      ...
      if (
        ...
        promises.push(
          axios
            .get(
              "/api/v1/patient_system/out_patient/obstetrical/ultrasound_id_for_admission_id_for_cpn/" +
                this.formData.consultation_id
            )
            .then((response) => {
              if (response.data) {
                this.formData.ultrasound_admission_id = response.data;
                this.linked_ultrasound_change(
                  this.formData.ultrasound_admission_id
                );
              }
            })
        );
    ...
    linked_ultrasound_change(val) {
      ...
      this.accessory.loading.ultrasound = true;
      this.accessory.linked_ultrasound_exists = false;

      axios
        .get(
          "/api/v1/patient_system/out_patient/obstetrical/ultrasound/" +
            this.formData.ultrasound_admission_id
        )
        .then((response) => {
          this.accessory.linked_ultrasound_exists = true;
          this.accessory.loading.ultrasound = false;
          if (response.data.patient_id != this.formData.patient_id) {
            this.accessory.ultrasound_link_error_message =
              "Patient Id does not match referenced ultrasound patient Id";
          } else {
        ...
        .catch((error) => {
          ...
          this.accessory.linked_ultrasound_exists = false;
          this.accessory.ultrasound_link_error_message =
            "Could not retrieve the ultrasound data";
        });
    },
    set_ultrasound_data(data) {
      let screening;
      // only the first and early screenings have calc.
      let first_screening;

      if (data.first && data.first.length > 0) {
        const first = data.first[0];
        first_screening = first[first.length - 1];
      } else if (data.early && data.early.length > 0) {
        first_screening = data.early[0];
      }

      if (
        data.confirmation_of_due_date &&
        data.confirmation_of_due_date.length > 0
      ) {
        screening = data.confirmation_of_due_date[0];
      } else {
        screening = first_screening;
      }

      this.ultrasound_data.unknown_lpd = first_screening.unknown_lpd;
      this.ultrasound_data.ddr = first_screening.ddr;
      this.ultrasound_data.edd_calc = first_screening.edd_calculated;

      this.ultrasound_data.created_at = screening.created_at;
      this.ultrasound_data.edd_method = screening.edd_method;
      this.ultrasound_data.edd_ultrasound = screening.edd_ultrasound;
      this.ultrasound_data.edd_corrected = screening.edd_corrected;
      ...
      this.formData.planned_oc_date = this.planned_oc_default_date;
    },
  ...
  computed: {
    isLoading() {
      return (
        this.accessory.loading.data ||
        this.accessory.loading.patient ||
        this.accessory.loading.ultrasound
      );
    },
    canEdit() {
