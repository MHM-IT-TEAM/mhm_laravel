                ...
                <template v-slot:top>
                    ...
                    >
                        <v-toolbar-title>PRENATAL CARE FOLLOWUP</v-toolbar-title>
                            ...
                            <v-spacer></v-spacer>
                        <v-btn color="primary" @click="navigateToAdmission" :disabled="!canShowData" v-if="!is_overview">View first checkup</v-btn >
                        <v-divider
                            class="mx-4"
                            inset
                            vertical
                            v-if="!is_overview"
                        ></v-divider>
                        <v-btn color="primary" @click="navigateToOverview" :disabled="!canShowData" v-if="!is_overview">View overview</v-btn>
                        <v-divider
                            class="mx-4"
                            inset
                            vertical
...
const {
    ...
    export default {
        ...
        computed: {
            ...
            canShowData() {
                return this.reference != null && this.reference != '' && this.accessory.data_fetched && !this.noDataFound && this.patient_details && (this.is_overview !== true);
            },
            patientFullName() {
        ...
        watch: {
            '$route.params': {
                handler: async function() {
                    if (!this.accessory.initialized || this.$route.params.cpn_ref == this.reference || !this.$route.params.cpn_ref)
                        return;

                    this.reference = this.$route.params.cpn_ref;
        ...
        methods: {
            async initialize () {
                ...
                else if (this.$route.params.cpn_ref) {
                    this.reference = this.$route.params.cpn_ref;
                    this.admission_id = this.$route.params.admission_id ?? null;

                    history.replaceState(null, null, this.$router.resolve({ name: 'cpn_followup' }).href);
                }
            ...
            async deleteItemConfirm () {
                await axios.delete(`/api/v1/patient_system/out_patient/obstetrical/cpn_followup/${this.cpn_data[this.editedIndex].id}`)
            ...
            async save () {
                    if (this.editedIndex > -1) {
                        ...
                        let update = await axios.put(`/api/v1/patient_system/out_patient/obstetrical/cpn_followup/${this.cpn_data[this.editedIndex].id}`,this.editedItem)
                        ...
                        this.editedItem.cpn_admission_id=this.reference;
                        this.editedItem.admission_id = this.admission_id;
                        this.editedItem.responsible=window.auth.user.name;
                        if (!this.$v.$invalid) {
                            let post= await axios.post('/api/v1/patient_system/out_patient/obstetrical/cpn_followup',this.editedItem)
            ...
            async search(){
                ...
                let fetch = await axios.get(`/api/v1/patient_system/out_patient/obstetrical/cpn_followup/${this.reference}`)
                    .catch(ex => {
                        console.log(ex);
                    });

                if(fetch && fetch.data && fetch.data.patient){
                    ...
                    this.first_checkup = fetch.data.first_checkup;
                    this.patient_details = fetch.data.patient;
                    this.editedItem.gestational_age 
                        = this.defaultItem.gestational_age 
                        = this.accessory.current_gestational_age 
                        = fetch.data.ga;
                    this.accessory.edd = fetch.data.edd;
                    this.accessory.edd_method = fetch.data.edd_method;

                    this.accessory.completed_screenings = fetch.data.completed_screenings;
                    this.accessory.first_screening_ga = fetch.data.first_screening_ga;

                    this.set_us_needed_if_necessary();
                }
                else {
                    this.noDataFound=true;
                }
                this.accessory.isLoading = false;
                this.accessory.data_fetched = true;
            },
            set_us_needed_if_necessary() {
                if (this.editedItem.gestational_age) {
                    const ga_in_days = this.get_gestational_age_in_days(this.editedItem.gestational_age);
                    const first_ga = [56, 83]; // 8+0 to 11+6
                    const second_ga = [126, 153]; // 18+0 to 21+6
                    const third_ga = [196, 223]; // 28+0 to 31+6

                    let actual_first_ga = this.get_gestational_age_in_days(this.accessory.first_screening_ga);

                    let first_done = this.accessory.completed_screenings.first;
                    let cdd_done = this.accessory.completed_screenings.cdd;
                    let second_done = this.accessory.completed_screenings.second;
                    let third_done = this.accessory.completed_screenings.third;

                    let us_explanation = null;
                    if (!first_done && ga_in_days >= first_ga[0] && ga_in_days <= first_ga[1]) {
                        us_explanation = "First screening";
                    }
                    else if (actual_first_ga > first_ga[1] && !cdd_done && !second_done && ga_in_days >= (actual_first_ga + 14) && ga_in_days < second_ga[0]) {
                        us_explanation = "Confirmation of due date screening";
                    }
                    else if (!second_done && ga_in_days >= second_ga[0] && ga_in_days <= second_ga[1]) {
                        us_explanation = "Second screening";
                    }
                    else if(!third_done && ga_in_days >= third_ga[0] && ga_in_days <= third_ga[1]) {
                        us_explanation = "Third screening";
                    }

                    if (us_explanation) {
                        this.editedItem.us_needed = this.defaultItem.us_needed = '1';
                        this.editedItem.us_reason = this.defaultItem.us_reason = "QM";
                        this.editedItem.us_explanation = this.defaultItem.us_explanation = us_explanation;
                    }
                }
            },
